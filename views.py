...
def pac(request):
	if request.method == 'POST':
		if request.method == 'POST':
			...
			from sklearn.ensemble import RandomForestClassifier
			from sklearn.tree import DecisionTreeClassifier
			df = pd.read_csv(r'healthcare_stroke.csv')
			df1=df.fillna(0)
			...
			atest=[[headline1,headline2,headline3,headline4,headline5,headline6,headline7,headline8,headline9,headline10]]
			#train_test separation
			X_train,X_test,y_train,y_test = train_test_split(X,y,test_size = 0.2)
			...
			pred_final123 = model_1.predict(X_test)
			pred_final12 = model_1.predict(atest)
				 
		
				 
			# printing log loss between actual and predicted value
			print('000000000000000000000000000000000')
			print(metrics.accuracy_score(y_test, pred_final123))
			...
			d={'predictedvalue':hd,'accuracy':metrics.accuracy_score(y_test, pred_final123)}				   
